...
function mapServerGraphToClient(g: ServerGraph): GraphData {
  // console.log("mapping server graph to client graph");
  // console.log(g);
...
export const useGraphStore = create<GraphState>((set, get) => ({
  ...
  setMode: (m) =>
    set({
      mode: m,
      pendingSource: m === "connect" ? null : get().pendingSource,
    }),
  ...
  addLink: (sourceId, targetId) =>
    set((s) => {
      ...
      const exists = s.graph.links.some(
        (l) =>
      ...
      const g = {
        ...
        links: [
          ...s.graph.links,
          { id: uid(), source: sourceId, target: targetId },
        ],
      };
      return { graph: recomputeDegrees(g) };
    }),
  ...
  addFreeNodeAt: (x, y) =>
    set((s) => {
      ...
      const node: KGNode = {
        ...
        y,
        fx: x,
        fy: y, // pin so it stays where you clicked
      };
      const g: GraphData = {
  ...
  expandNode: (id) =>
    set((s) => {
      ...
      const newLinks = newNodes.map((ch) => ({
        id: uid(),
        source: node.id,
        target: ch.id,
      }));
      ...
      const g: GraphData = {
        nodes: s.graph.nodes
          .map((n) => (n.id === id ? { ...n, expanded: true } : n))
          .concat(newNodes),
